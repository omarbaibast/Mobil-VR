...
function updateGamepad() {
    ...
    if (gamepad) {
        if (statusEl) statusEl.innerText = "Controller: " + gamepad.id;
        if (instructionEl) instructionEl.style.display = 'none';

        if (debugEl) {
            const activeButtons = gamepad.buttons
                .map((b, i) => (b.pressed || b.value > 0.1)
                    ? `<span style="color:#00ff00;font-weight:bold;">[${i}]</span>`
                    : `[${i}]`)
                .join(' ');
            ...
            const activeAxes = gamepad.axes
                .map((a, i) =>
                    `<div style="display:inline-block;width:60px;">
                     A${i}:<span style="color:${Math.abs(a)>0.1?'#00ff00':'#888'}">${a.toFixed(2)}</span>
                     </div>`)
                .join(' ');
        ...
        if (statusEl) statusEl.innerText = "Controller: Press any button";
...
function render() {
    ...
    updateGamepad();  // ✅ reliable gamepad detection
    updateGame();
    ...
    if (gamepad) {
        ...
        if (Math.abs(rsX) > deadzone) playerGroup.rotation.y -= rsX * ROTATION_SPEED;
        ...
        // Shoot with any pressed button
        const isShooting = gamepad.buttons.some(b => b.pressed || b.value > 0.1);
        if (isShooting) shoot();
